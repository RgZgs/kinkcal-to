...
import { getUpcomingEvents, getCategories } from './db';
...
export async function generateICS(): Promise<string> {
  ...
  const icsEvents = events.map((e: any) => {
    const slugs: string[] = e.category_slugs ? e.category_slugs.split(',') : [];
    const emojis: string[] = e.category_emojis ? e.category_emojis.split(',') : [];
    ...
    if (e.recurrence_label || e.recurrence_note) {
      descLines.push(`Recurrence: ${e.recurrence_label || e.recurrence_note}`);
    }
